...
// Build query
$where = "WHERE sk.tanggal BETWEEN '$tanggal_mulai' AND '$tanggal_akhir'";

if ($search) {
    $search = escape($search);
    $where .= " AND (sk.no_transaksi LIKE '%$search%' OR sk.tujuan LIKE '%$search%')";
}
...
$countResult = fetchOne("SELECT COUNT(*) as total FROM stok_keluar sk $where");
$totalItems = $countResult['total'];
$totalPages = ceil($totalItems / $limit);

$items = fetchAll("SELECT sk.*, u.nama_lengkap as user_nama,
...
// Calculate summary
$summary = fetchOne("SELECT COUNT(*) as total_trx, SUM(total_nilai) as total_nilai FROM stok_keluar sk $where");
?>

<script>setPageTitle('Stok Keluar', 'Riwayat pengeluaran stok');</script>
...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-white">Stok Keluar</h1>
    ...
    <a href="/gudang-gizi/modules/stok_keluar/create.php"
        class="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 px-4 py-2 rounded-xl text-white font-medium flex items-center gap-2 w-fit transition-all">
    ...
    <form method="GET" class="grid md:grid-cols-4 gap-4">
        ...
        <div>
            ...
            <input type="text" name="search" value="<?= htmlspecialchars($search) ?>"
                class="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm"
                placeholder="No. transaksi atau tujuan...">
        </div>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <?php if ($item['status'] === 'completed'): ?>
                                        <a href="/gudang-gizi/modules/stok_keluar/print.php?id=<?= $item['id'] ?>" target="_blank"
    ...
    <?php if ($totalPages > 1): ?>
        <div class="px-4 py-4 border-t border-slate-700 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-1">
                <?php if ($page > 1): ?>
                    <a href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&tanggal_mulai=<?= $tanggal_mulai ?>&tanggal_akhir=<?= $tanggal_akhir ?>"
                ...
                <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
                    <a href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&tanggal_mulai=<?= $tanggal_mulai ?>&tanggal_akhir=<?= $tanggal_akhir ?>"
